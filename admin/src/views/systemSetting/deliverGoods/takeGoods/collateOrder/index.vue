  ...
  <div class="divBox">
        ...
        <div class="container">
          <el-form size="small" label-width="100px" :inline="true">
            ...
            <el-form-item label="选择门店：">
              <el-select v-model="tableFrom.storeId" filterable placeholder="请选择" class="selWidth" clearable
                @change="getList(1)">
    ...
    <el-card class="box-card">
      <el-table v-loading="listLoading" :data="tableData.data" style="width: 100%" size="mini" class="table"
        highlight-current-row :header-cell-style="{ fontWeight: 'bold' }">
              ...
              <div v-if="scope.row.productList && scope.row.productList.length" slot="reference">
                <div v-for="(val, i ) in scope.row.productList" :key="i" class="tabBox acea-row row-middle"
                  ...
                  <div class="text_overflow">
                    ...
                    <span class="tabBox_pice">{{ '￥' + val.info.price ? val.info.price + ' x ' + val.info.payNum : '-'
                    }}</span>
                  </div>
                </div>
              </div>
        ...
        <el-table-column label="操作" min-width="150" fixed="right">
          <template slot-scope="scope">
            <el-button type="text" size="small" @click.native="onOrderDetails(scope.row.orderId)"  v-hasPermi="['admin:order:info']">订单详情</el-button>
            <el-button
              v-if="scope.row.statusStr.key === 'toBeWrittenOff' "
              type="text" size="small" v-hasPermi="['admin:order:write:update']"
              @click="onWriteOff(scope.row)">立即核销</el-button>
...
import detailsFrom from '@/views/order/orderDetail.vue'
export default {
  components: { cardsData,detailsFrom },
  data() {
    return {
      storeSelectList: [],
      orderId: 0,
