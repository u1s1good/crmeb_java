  ...
  <div class="divBox">
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <el-button size="small" type="primary" @click="onAdd(null)"
          v-hasPermi="['admin:user:group:save', 'admin:user:tag:save']">{{ $route.path.indexOf('group') !==
            -1 ? '添加用户分组' : '添加用户标签' }}</el-button>
      </div>
      <el-table v-loading="listLoading" :data="tableData.data" style="width: 100%" size="small">
        ...
        <el-table-column label="操作" min-width="120" fixed="right" align="center">
          <template slot-scope="scope">
            <el-button class="mr10" type="text" size="small" @click="onAdd(scope.row)"
              v-hasPermi="['admin:user:group:update', 'admin:user:tag:update']">编辑</el-button>
            <el-button type="text" size="small" @click="handleDelete(scope.row.id, scope.$index)" disable
              v-hasPermi="['admin:user:group:delete', 'admin:user:tag:delete']">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <div class="block">
